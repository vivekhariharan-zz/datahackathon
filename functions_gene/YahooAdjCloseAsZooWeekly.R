YahooAdjCloseAsZooWeekly = function(symbols, ...){
    require('zoo')
    require('fImport')
...
if(FALSE){
    ...
    newprices = YahooAdjCloseAsZooWeekly(symbols, begin='2005-01-01')
    
    
    plot(newprices)
    
    
    wtt(cbind(as.character(as.Date(index(newprices))), coredata(newprices)))
    wtt(cbind(as.character(as.Date(index(orig))), coredata(orig)))
