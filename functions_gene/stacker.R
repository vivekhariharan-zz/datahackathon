# Stacker
# 9/11/2009
# 12/2/2010 - added example
# Author: gene.leynes
###############################################################################


#stacker=function(x,id=NULL,vars=NULL,labs=c('variable','value')){
##	browser()
#	if(is.null(id)|is.null(vars))stop('Please specify ids and vars')
#	temp1=c()
#	temp2=c()
#	for (i in seq(length(vars))){
#		temp1=c(temp1,rep(colnames(x)[vars[i]],nrow(x)))
#		temp2=c(temp2,x[,vars[i]])
#	}
#	if(any(is.na(id)))return(temp2)
#	rownames(x)=NULL
#	ret=data.frame(x[,id],variable=temp1,value=temp2)
#	if(is.character(id)){
#		names(ret)=c(id,labs)
#	}else{
