...
#Average over hours
target_means_by_hour_of_day = 
	ddply(TrainingData, "hour", function(df) colMeans(df[,target_names], na.rm = TRUE))
...
replaceNAs = function(submission, sample) {
	submission = submission[,colnames(sample)]
	stopifnot(mean(names(submission) == colnames(sample))==1)
...
target_means_by_hour_and_chunkID = 
	ddply(TrainingData, c("chunkID", "hour"), function(df) colMeans(df[,target_names], na.rm = TRUE))
names(SubmissionZeros)
